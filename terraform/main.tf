...
# SQS Queue for request queuing
resource "aws_sqs_queue" "query_queue" {
  name                      = "${var.cluster_name}-query-queue"
  delay_seconds             = 0
...
resource "aws_s3_bucket" "model_artifacts" {
  bucket = "${var.cluster_name}-model-artifacts-${random_id.bucket_suffix.hex}"
}
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(module.eks.cluster_oidc_issuer_url, "https://", "")}:sub": "system:serviceaccount:default:llm-service-account"
